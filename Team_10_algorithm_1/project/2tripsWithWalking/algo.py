...
def merge(trips):
    n=len(trips)
    url='http://localhost:5000/route/v1/driving/'+'-73.7781,40.6413'+';'+trips[0].dropoff_longitude+','+trips[0].dropoff_lattitude+'?overview=false' 
    r=requests.get(url)
    try:
        json_data=r.json()
    except:
    ...
    data=dict(json_data['routes'][0])
    d1=data['distance']
    url='http://localhost:5000/route/v1/driving/'+'-73.7781,40.6413'+';'+trips[1].dropoff_longitude+','+trips[1].dropoff_lattitude+'?overview=false' 
    r=requests.get(url)
    try:
        json_data=r.json()
    except:
    ...
    d2=data['distance']
    if(d1<d2):
        if(trips[0].willing_to_walk==1):
            bearing=walking.calculate_bearing(trips[0].dropoff_lattitude, trips[0].dropoff_longitude, 40.6413, -73.7781)
            new1_lat,new1_long=walking.lat_long(bearing,trips[0].dropoff_lattitude, trips[0].dropoff_longitude)
            temp1_lat,temp1_long=trips[0].dropoff_lattitude, trips[0].dropoff_longitude
            trips[0].dropoff_lattitude, trips[0].dropoff_longitude=new1_lat,new1_long
        else:
            temp1_lat,temp1_long=trips[0].dropoff_lattitude, trips[0].dropoff_longitude
            new1_lat,new1_long=trips[0].dropoff_lattitude, trips[0].dropoff_longitude
        if(trips[1].willing_to_walk==1):
            bearing=walking.calculate_bearing(trips[1].dropoff_lattitude, trips[1].dropoff_longitude, new1_lat,new1_long)
            new2_lat,new2_long=walking.lat_long(bearing,trips[1].dropoff_lattitude, trips[1].dropoff_longitude)
            temp2_lat,temp2_long=trips[1].dropoff_lattitude, trips[1].dropoff_longitude
            trips[1].dropoff_lattitude, trips[1].dropoff_longitude=new2_lat,new2_long
        else:
            temp2_lat,temp2_long=trips[1].dropoff_lattitude, trips[1].dropoff_longitude
            new2_lat,new2_long=trips[1].dropoff_lattitude, trips[1].dropoff_longitude   
    else:
        if(trips[1].willing_to_walk==1):
            bearing=walking.calculate_bearing(trips[1].dropoff_lattitude, trips[1].dropoff_longitude, 40.6413, -73.7781)
            new2_lat,new2_long=walking.lat_long(bearing,trips[1].dropoff_lattitude, trips[1].dropoff_longitude)
            temp2_lat,temp2_long=trips[1].dropoff_lattitude, trips[1].dropoff_longitude
            trips[1].dropoff_lattitude, trips[1].dropoff_longitude=new2_lat,new2_long
        else:
            ...
            new2_lat,new2_long=trips[1].dropoff_lattitude, trips[1].dropoff_longitude
        if(trips[0].willing_to_walk==1):
            bearing=walking.calculate_bearing(trips[0].dropoff_lattitude, trips[0].dropoff_longitude, new2_lat,new2_long)
            new1_lat,new1_long=walking.lat_long(bearing,trips[0].dropoff_lattitude, trips[0].dropoff_longitude)
            temp1_lat,temp1_long=trips[1].dropoff_lattitude, trips[0].dropoff_longitude
            trips[0].dropoff_lattitude, trips[0].dropoff_longitude=new1_lat,new1_long
        else:
            temp1_lat,temp1_long=trips[0].dropoff_lattitude, trips[0].dropoff_longitude
            new1_lat,new1_long=trips[0].dropoff_lattitude, trips[0].dropoff_longitude
    ...
    if(delay>20):
        return False,0,0
        trips[0].dropoff_lattitude, trips[0].dropoff_longitude=temp1_lat,temp1_long
        trips[1].dropoff_lattitude, trips[1].dropoff_longitude=temp2_lat,temp2_long
    else:
    ...
    for i in range(len(pool1)):
        for j in range(len(pool3)):
            if((pool3[j].trip_id not in three_to_one.keys()) and (pool1[i].trip_id not in three_to_one.values())):
                boolean,saved,distance_of_merged_trips=merge([pool1[i],pool3[j]])                
                if(boolean==True):
    ...
    for i in range(len(pool2)):
        for j in range(len(pool2)):
            if(i!=j and pool2[i].trip_id not in two_to_two.keys() and pool2[i].trip_id not in two_to_two.values() and pool2[j].trip_id not in two_to_two.keys() and pool2[j].trip_id not in two_to_two.values()):
                boolean,saved,distance_of_merged_trips=merge([pool2[i],pool2[j]])                
                if(boolean==True):
    ...
    for i in range(len(pool1)):
        for j in range(len(pool2)):
            if((pool2[j].trip_id not in two_to_one.keys()) and (pool1[i].trip_id not in two_to_one.values())):
                boolean,saved,distance_of_merged_trips=merge([pool1[i],pool2[j]])              
                if(boolean==True):
    ...
    for i in range(len(pool1)):
        for j in range(len(pool1)):
            if(i!=j and pool1[i].trip_id not in one_to_one.keys() and pool1[i].trip_id not in one_to_one.values() and pool1[j].trip_id not in one_to_one.keys() and pool1[j].trip_id not in one_to_one.values()):
                boolean,saved,distance_of_merged_trips=merge([pool1[i],pool1[j]])               
                if(boolean==True):
